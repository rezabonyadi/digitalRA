...
def load_write_review_page(llm_object=None):
...
4. In case of any issue, try "Renew connection"."
    ...
    estimated_cost = llm_object.get_estimated_cost(concated_data, litrature_review_len)
    ...
    if button_initiate_review:
        with st.spinner('Writing the review'):
            st.session_state['litrature_review'] = operations.write_litrature_review(working_dir, llm_object, researcher_spec, idea_text_summary, papers_df, concated_data)

            # st.toast(f'Digital RA> Total cost: {str(llm_object.get_current_cost()+short_context_model.get_current_cost())}')
            print('--------------')
